...
tailmessage_file = tailmessage_config.get("TailmessagePath", fallback="./SOUNDS/wx-tail.wav")
...
def getAlerts(countyCodes):
    ...
    if config.getboolean('DEV', 'INJECT', fallback=False):
        ...
        alerts = [alert.strip() for alert in config["DEV"].get("INJECTALERTS").split(",")]
...
def main():
    ...
    if old_alerts != alerts:
        ...
        ct_alerts = [alert.strip() for alert in config["CourtesyTones"].get("CTAlerts").split(",")]
        ...
        if enable_ct_auto_change:
            logger.debug(f"CT auto change is enabled, alerts that require a CT change: {ct_alerts}")
            # Check if any alert matches ct_alerts
            if set(alerts).intersection(ct_alerts):
                for alert in alerts:
                    if alert in ct_alerts:
                        logger.debug(f"Alert {alert} requires a CT change")
                ...
                logger.debug(f"No alerts require a CT change, reverting to normal.")
